...
# 对于异常的测试可使用 assertRaises() 方法。 例如，如果你想测试某个函数抛出了 ValueError
# 异常，像下面这样写：
...
# 这种方法的问题在于它很容易遗漏其他情况，比如没有任何异常抛出的时候。 那么你还得需要增加另外的
# 检测过程，如下面这样：
class TestConversion(unittest.TestCase):
    def test_bad_int(self):
        try:
...
#
# assertRaises() 的一个缺点是它测不了异常具体的值是多少。 为了测试异常值，可以使用
# assertRaisesRegex() 方法， 它可同时测试异常的存在以及通过正则式匹配异常的字符串表示。例如：
class TestConversion(unittest.TestCase):
    def test_bad_int(self):
