...
# __set__() 和 __delete__() 这三个特殊的方法。 这些方法接受一个实例作为输入，之后相应的操作
# 实例底层的字典。
#
...
# 作为输入，描述器的每一个方法会接受一个操作实例。 为了实现请求操作，会相应的操作实例底层的字典
# (__dict__属性)。 描述器的 self.name 属性存储了在实例字典中被实际使用到的key。


# 描述器可实现大部分Python类特性中的底层魔法， 包括 @classmethod 、@staticmethod 、
# @property ，甚至是 __slots__ 特性。

# 通过定义一个描述器，你可以在底层捕获核心的实例操作(get, set, delete)，并且可完全自定义
# 它们的行为。 这是一个强大的工具，有了它你可以实现很多高级功能，并且它也是很多高级库和框架中的
# 重要工具之一。
#
# 描述器的一个比较困惑的地方是它只能在类级别被定义，而不能为每个实例单独定义。因此，下面的代码是
# 无法工作的：
class Point:
    def __init__(self, x, y):
...
# __get__() 看上去有点复杂的原因归结于实例变量和类变量的不同。 如果一个描述器被当做一个类变量
# 来访问，那么 instance 参数被设置成 None 。 这种情况下，标准做法就是简单的返回这个描述器本
# 身即可(尽管你还可以添加其他的自定义操作)。例如：
...
# 这种情况下property技术会更加容易。 当程序中有很多重复代码的时候描述器就很有用了
