...
# 如果你想监控类的定义，通常可以通过定义一个元类。一个基本元类通常是继承自 type 并重定义它的
# __new__() 方法 或者是 __init__() 方法。比如：
...
#
# 作为一个具体的应用例子，下面定义了一个元类，它会拒绝任何有混合大小写名字作为方法的类定义（可能是
# 想气死Java程序员^_^）：
...
# 作为更高级和实用的例子，下面有一个元类，它用来检测重载方法，确保它的调用参数跟父类中原始方法有着
# 相同的参数签名。
...
# 在大型面向对象的程序中，通常将类的定义放在元类中控制是很有用的。 元类可以监控类的定义，警告编程人
# 员某些没有注意到的可能出现的问题。
#
...
# 方法是在类被创建之后被调用，当你需要完整构建类对象的时候会很有用。 在最后一个例子中，这是必要的，
# 因为它使用了 super() 函数来搜索之前的定义。
# 它只能在类的实例被创建之后，并且相应的方法解析顺序也已经被设置好了。
#
# 最后一个例子还演示了Python的函数签名对象的使用。 实际上，元类将每个可调用定义放在一个类中，
# 搜索前一个定义（如果有的话）， 然后通过使用
# inspect.signature() 来简单的比较它们的调用签名。
#
# 最后一点，代码中有一行使用了 super(self, self) 并不是排版错误。 当使用元类的时候，
# 我们要时刻记住一点就是 self
