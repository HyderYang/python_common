...
# 可以看出，最后抛出了一个NameError异常，就跟在 exec() 语句从没执行过一样。 要是你想在后面的计
# 算中使用到 exec()
# 执行结果的话就会有问题了。
#
...
# 实际上对于 exec() 的正确使用是比较难的。大多数情况下当你要考虑使用 exec() 的时候， 还有另外
# 更好的解决方案（比如装饰器、闭包、元类等等）。
#
# 然而，如果你仍然要使用 exec() ，本节列出了一些如何正确使用它的方法。 默认情况下，exec()
# 会在调用者局部和全局范围内执行代码。然而，在函数里面， 传递给 exec() 的局部范围是拷贝实际局部
# 变量组成的一个字典。 因此，如果 exec()
...
#
# 在使用 locals() 的时候，你需要注意操作顺序。每次它被调用的时候， locals() 会获取局部变量值中
# 的值并覆盖字典中相应的变量。
# 请注意观察下下面这个试验的输出结果：
def test3():
    x = 0
...
# 大部分情况下，这种方式是使用 exec() 的最佳实践。 你只需要保证全局和局部字典在后面代码访问时已经
# 被初始化。
#
# 还有一点，在使用 exec() 之前，你可能需要问下自己是否有其他更好的替代方案。 大多数情况下当你要
# 考虑使用 exec() 的时候，
