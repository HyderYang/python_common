#!/usr/bin/env python
# -*- encoding: utf-8 -*-
# 想在装饰器中给被包装函数增加额外的参数，但是不能影响这个函数现有的调用规则。
...
# 通过使用强制关键字参数，它被作为一个特殊情况被挑选出来， 并且接下来仅仅使用剩余的位置和关键字参数
# 去调用这个函数时，这个特殊参数会被排除在外。
# 也就是说，它并不会被纳入到 **kwargs 中去。
#
# 还有一个难点就是如何去处理被添加的参数与被包装函数参数直接的名字冲突。 例如，如果装饰器
# @optional_debug 作用在一个已经拥有一个
