#!/usr/bin/env python
# -*- encoding: utf-8 -*-
# 代码中需要依赖到回调函数的使用(比如事件处理器、等待后台任务完成后的回调等)， 并且你还需要
# 让回调函数拥有额外的状态值，以便在它的内部使用到。

# 回调函数的使用——特别是跟异步处理有关的。 为了演示与测试，我们先定义如下一个需要调用回调函数的函数
...
#
# 为了让回调函数访问外部信息，一种方法是使用一个绑定方法来代替一个简单函数。 比如，
# 下面这个类会保存一个内部序列号，每次接收到一个 result
# 的时候序列号加1：
...
#
# 至少有两种主要方式来捕获和保存状态信息，你可以在一个对象实例(通过一个绑定方法)或者在一个
# 闭包中保存它。
...
#
# 而使用一个协程来作为一个回调函数就更有趣了，它跟闭包方法密切相关。 某种意义上来讲，
# 它显得更加简洁，因为总共就一个函数而已。
# 并且，你可以很自由的修改变量而无需去使用 nonlocal 声明。 这种方式唯一缺点就是相对于其他
# Python技术而言或许比较难以理解。
