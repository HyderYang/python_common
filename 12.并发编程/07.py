...
# concurrent.futures 函数库有一个 ThreadPoolExecutor 类可以被用来完成这个任务。 下面是
# 一个简单的TCP服务器，使用了一个线程池来响应客户端：
import threading
...
# 使用 ThreadPoolExecutor 相对于手动实现的一个好处在于它使得 任务提交者更方便的从被调用函数
# 中获取返回值。例如，你可能会像下面这样写
def fetch_url(url):
...
# 例子中返回的handle对象会帮你处理所有的阻塞与协作，然后从工作线程中返回数据给你。 特别的，
...
#
# 你可能会关心创建大量线程会有什么后果。 现代操作系统可以很轻松的创建几千个线程的线程池。 甚至，
# 同时几千个线程等待工作并不会对其他代码产生性能影响。
...
# 放置线程的执行栈（通常是8MB大小）。但是这个内存只有一小片段被实际映射到真实内存中。 因此，
# Python进程使用到的真实内存其实很小
...
# 210MB的虚拟内存，而真实内存使用量没有变。 注意线程栈大小必须至少为32768字节，通常是系统内存
# 页大小（4096、8192等）的整数倍。
